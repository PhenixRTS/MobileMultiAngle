...
internal class PhenixRoomRepository(
    ...
    fun joinRoom(configuration: PhenixRoomConfiguration) {
        ...
        roomExpress.joinRoom(configuration) { service ->
            ...
            selfCoreMember = PhenixCoreMember(roomService!!.self, true).apply {
    ...
    fun createRoom(configuration: PhenixRoomConfiguration) {
        _onEvent.tryEmit(PhenixEvent.PHENIX_ROOM_CREATING)
        Timber.d("Creating room with configuration: $configuration")
        roomExpress.createRoom(configuration) { status ->
            if (status == RequestStatus.OK) {
                _onEvent.tryEmit(PhenixEvent.PHENIX_ROOM_CREATED)
            } else {
                _onError.tryEmit(PhenixError.CREATE_ROOM_FAILED)
            }
        }
    }
    ...
    fun publishToRoom(alias: String, role: MemberRole, name: String?) {
        ...
        val publishOptions = getPublishOptions(
            userMediaStream!!,
            configuration.publishToken ?: configuration.edgeToken,
            emptyList()
        )
