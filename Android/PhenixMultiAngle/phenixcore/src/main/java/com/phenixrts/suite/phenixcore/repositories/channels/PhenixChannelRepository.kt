...
internal class PhenixChannelRepository(
    ...
    fun joinAllChannels(channelAliases: List<String>) {
        if (configuration.streamTokens.isNotEmpty() && configuration.streamTokens.size != channelAliases.size) {
        ...
        channelAliases.forEachIndexed { index, channelAlias ->
            ...
            channel.join(
                PhenixChannelConfiguration(
                    ...
                    streamToken = configuration.streamTokens.getOrNull(index) ?: configuration.edgeToken,
